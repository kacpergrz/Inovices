...
const SERVICE_ROLE_KEY =
  APP_SUPABASE_SERVICE_ROLE_KEY || tryExtractFirstSecretKey(SUPABASE_SECRET_KEYS);
...
function tryExtractFirstSecretKey(input: string): string {
  if (!input) return "";

  try {
    const parsed = JSON.parse(input);
    if (Array.isArray(parsed) && parsed.length > 0) {
      return parsed[0]?.secret ?? "";
    }
    if (parsed && typeof parsed === "object") {
      const first = Object.values(parsed)[0] as any;
      if (typeof first === "string") return first;
      if (first && typeof first === "object" && typeof first.secret === "string") {
        return first.secret;
      }
    }
  } catch {
